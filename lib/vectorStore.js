...
class VectorStore {
    ...
    /**
     * Load validated vectors filtered by agent scope.
     * Returns vectors that are either global or belong to the current agent.
     * Falls back to loading all vectors if no agentId provided (backward compat).
     *
     * @param {string} currentAgentId - Agent ID to filter by
     * @returns {Array} Filtered validated vectors
     */
    loadVectorsForAgent(currentAgentId) {
        const data = this.loadFile();
        return data.vectors.filter(v =>
            v.validation_status === 'validated' || v.validation_status === 'integrated'
        ).filter(v =>
            v.scope === 'global' || v.agentId === currentAgentId || !v.agentId // fallback für alte vectors
        );
    }
     ...
     * @param {string} currentAgentId - Agent ID for vector scoping (default: 'main')
     * @returns {Array} Top vectors above threshold, max `maxInjected`
     */
    getRelevantVectors(userMessage = '', entropyScore = 0, currentAgentId = 'main', options = {}) {
        const vectors = this.loadVectorsForAgent(currentAgentId);
